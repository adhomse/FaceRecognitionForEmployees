...
def detect_faces(img, mtcnn):
    ...
    img_size = np.asarray(img.shape)[0:2] #480,640
    ...
    if nrof_bb > 0:
        ...
        for i in range(nrof_bb):
            ...
            x=img[bb[1]:bb[3]]
            print("x",x)
            print("cropped",cropped,len(x))
            aligned = misc.imresize(cropped, (image_size, image_size), interp='bilinear') 
            print("aligned",aligned,len(aligned))

            prewhitened = prewhiten(aligned)
            padded_bounding_boxes.append(bb)
            face_patches.append(prewhitened) #it append 160 facepatches
            print("----------------")
            print("facePtaches",face_patches)
            for i in face_patches:
                print(len(i))
